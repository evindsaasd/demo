...
<template>
  <div>
    ...
    <el-table size="small" :data="listData" highlight-current-row v-loading="loading" border element-loading-text="拼命加载中" style="width: 100%;">
      ...
      <el-table-column sortable prop="id" label="id" width="215" show-overflow-tooltip>
      </el-table-column>
      <el-table-column sortable prop="username" label="username" width="215" show-overflow-tooltip>
      </el-table-column>
      <el-table-column sortable prop="createTime" label="创建时间" width="300" show-overflow-tooltip>
      </el-table-column>
      <el-table-column sortable prop="updateTime" label="更新时间" width="300" show-overflow-tooltip>
      </el-table-column>
      <el-table-column style="text-align:center;" sortable prop="isForbidden" label="是否冻结" width="159">
        <template slot-scope="scope">
          <el-switch v-model="scope.row.isForbidden"  active-color="#13ce66"  inactive-color="grey"  :active-value=0  :inactive-value=1 >
          </el-switch>
          <el-button size="mini" @click="hand(scope.$index, scope.row)">更改状态</el-button>
        </template>
      </el-table-column>
      <!-- <el-table-column sortable prop="className" label="className" width="120" show-overflow-tooltip>
      </el-table-column> -->
      <!-- <el-table-column sortable prop="methodName" label="methodName" width="140" show-overflow-tooltip>
      </el-table-column> -->
      <!-- <el-table-column sortable prop="args" label="args" width="120" show-overflow-tooltip>
      </el-table-column> -->
      <!-- <el-table-column sortable prop="time" label="创建时间" width="180" show-overflow-tooltip>
        <template slot-scope="scope">
          <div>{{scope.row.addTime|timestampToTime}}</div>
        </template>
      </el-table-column> -->
      <!-- <el-table-column sortable prop="ip" label="ip" width="120" show-overflow-tooltip>
      </el-table-column> -->
...
<script>
import { getClient, reviseClient } from '../api/userMG'
...
export default {
  ...
  methods: {
    ...
    hand(index,row) {
      ...
      reviseClient("?forbidden="+this.fb+"&id="+row.id)
        .then(res => {
          if (res.code === 200) {
            this.getData()
            this.$message({
